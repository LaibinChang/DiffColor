...
class DenoisingDiffusion(object):
    ...
    def calculate_model_params_and_flops(self, input_shape):
         #如果模型使用 DataParallel，需要访问其内部模型
        if isinstance(self.model, torch.nn.DataParallel):
            self.model = self.model.module
            print("模型使用 DataParallel")
        print(f"输入大小为{input_shape}")
        # 计算参数量
        total_params = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        print(f"模型的参数量: {total_params}")

        # 构建输入张量，并将其移至模型所在设备
        input_tensor = torch.randn(input_shape).to(self.model.device)
        # 计算 FLOPs 和参数量
        flops, params = profile(self.model, inputs=(input_tensor,))
        print(f"模型的 FLOPs: {flops}")
        print(f"模型的参数量（使用 thop 计算）: {params}")

...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        input_LL_dwt = dwt(input_LL_LL)
        input_LL_LL, input_high2 = input_LL_dwt[:n, ...], input_LL_dwt[n:, ...]

        input_high2 = self.high_enhance1(input_high2)

        #三个增强后细节分量，对应元素求均值
        b, c, h, w = input_high1.shape
        input_high2_HL, input_high2_LH, input_high2_HH = input_high2[:b // 3, ...], input_high2[b // 3:2 * b // 3, ...], input_high2[2 * b // 3:, ...]
        ...
        if self.training:
            ...
            noise_output = self.Unet(torch.cat([input_LL_LL, x, mean_input_high2], dim=1), t.float())
            denoise_LL_LL = self.sample_training(input_LL_LL, b, mean_input_high2)
            ...
            denoise_LL_LL = self.sample_training(input_LL_LL, b, mean_input_high2)
            denoise_LL_LL = idwt(torch.cat((denoise_LL_LL, input_high2), dim=0))
            pred_LL = idwt(torch.cat((denoise_LL_LL, input_high1), dim=0))
            pred_x = idwt(torch.cat((pred_LL, input_high0), dim=0))
            pred_x = inverse_data_transform(pred_x)

...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        input_LL_LL, input_high1 = input_dwt[:n, ...], input_dwt[n:, ...]
        input_high1 = self.high_enhance0(input_high1)
        ...
        if self.training:
            ...
            gt_LL_LL, gt_high1 = gt_dwt[:n, ...], gt_dwt[n:, ...]
            ...
            noise_output = self.Unet(torch.cat([input_LL_LL, x, mean_input_high1], dim=1), t.float())
            denoise_LL_LL = self.sample_training(input_LL_LL, b, mean_input_high1)
            pred_x = idwt(torch.cat((denoise_LL_LL, input_high1), dim=0))

            """"""
            #pred_x = idwt(torch.cat((pred_LL, input_high1), dim=0))
